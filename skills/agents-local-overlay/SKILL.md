...
description: AGENTS.md(공용, commit 가능)와 AGENTS.local.md(개인, gitignore)를 Claude Code, Codex CLI, Kiro CLI(experimental)에서 로드되게 만드는 agent 중립 rule 파일 규약을 세팅하고 검증할 때 사용한다. git worktree, bare repo + worktree, 서브에이전트 로드 범위, 로컬 전용 지침 주입, CLAUDE.local.md와 Codex 동시 사용, Kiro steering materialize 요청에서 트리거한다.
---
...
| Claude subagent — built-in `Explore`·`Plan` | 제공 안 함 (Claude의 의도된 native memory 생략 예외, `additionalContext` 미사용) | 제공 안 함 (Claude의 의도된 native memory 생략 예외, `additionalContext` 미사용) |
...
| Kiro v3 (experimental) | native: default-resource 채널 | launcher가 생성하는 steering 문서 |

보증 문구: 안정 보증 대상은 Claude Code와 Codex CLI다. 각 native load 또는 Codex
hook invocation마다 현재 rule snapshot을 한 번 제공한다. Claude main
`SessionStart` hook은 rule을 주입하지 않고 생성
복사본 갱신과 notice만 담당한다. Claude built-in `Explore`·`Plan`의 native
memory 생략은 의도된 예외로 보고 overlay가 hook으로 보정하지 않는다. Claude
subagent hook context는 보증 대상이 아니다. resume·compact 후 이전 주입이 세션
history에 남아 생기는 중복은 보증 대상에서 제외한다. 채널의 전제조건이 확인되지
않으면 rule을 조용히 빼는 대신 `[agents-local-overlay] ...` notice를 context에
넣고, `verify`가 실패한다.
Kiro v3는 experimental 지원이며, Kiro를 target으로 쓸 때만 Kiro 관련 WARN/FAIL을
release gate로 본다.

## 전제

- `git` 2.36 이상과 `python3` 3.11 이상(`tomllib` 필요)이 필요하다.
- Claude Code 2.1.232, Codex CLI 0.147.0 이상을 안정 지원으로 전제한다.
  Kiro CLI 2.15.1(v3 engine) 이상은 experimental 지원이다.
  이 스킬이 쓰는 CLI 인터페이스(hook 이벤트, Kiro v3 engine·settings
...
- 세션 hook의 notice는 이 규약을 쓰는 repo에서만 나온다. `AGENTS.local.md`(또는
  ...
  범위다. Claude SubagentStart hook context는 쓰지 않는다.
...
- `SubagentStart` hook은 Claude 설정에 넣지 않는다. Claude의 subagent context
  hook 채널은 `hookSpecificOutput.additionalContext`이고, 이 overlay는
  `additionalContext`를 쓰지 않는다. built-in `Explore`·`Plan`은 빠르고 비용이
  낮은 탐색을 위해 native memory를 생략하는 Claude의 의도된 예외로 보고,
  overlay가 hook으로 보정하지 않는다. 해당 subagent에는 overlay rule이
  들어가지 않을 수 있다.
- `CLAUDE_CODE_DISABLE_CLAUDE_MDS=1`은 native 채널을 끄므로 지원하지 않는다.
  설정돼 있으면 hook이 notice만 낸다.
- 일반 Claude 설정의 두 key가 채널을 조용히 끊는다: `disableAllHooks`가
  true면 session notice/worktree hook 자체가 안 뜨고, `claudeMdExcludes`가
  `CLAUDE.md`/`CLAUDE.local.md` bridge 경로를 매칭하면 native bridge가 로드에서
  빠진다. 둘 다 런타임 hook으로는 잡히지 않으므로 `verify`가 Claude와 같은
  위치의 설정을 읽어 해당 시 FAIL한다: user는 `$CLAUDE_CONFIG_DIR/settings.json`
  (기본 `~/.claude`), project는 세션 시작 디렉터리의 `.claude/settings.json`
  (상위 디렉터리 fallback 없음), local은 main checkout root의
  `.claude/settings.local.json`이며 시작 디렉터리에 남은 local 파일도 root보다
  낮은 우선순위로 읽는다(bare layout은 각 worktree root). `verify`는 검사
  대상으로 준 디렉터리와 각 worktree root를 세션 시작점으로 보고 검사한다.
  `claudeMdExcludes` 판정은 절대경로와 `**`·`*`·`?` glob만 평가하고,
  brace·character class·extglob·negation이 든 패턴은 bridge가 살아 있음을
  증명할 수 없으므로 FAIL한다 — 절대경로나 `**`·`*`·`?` glob으로 다시 쓴다.
...
- repo(각 worktree 경로)를 Codex에서 trust한다. `trust_level = "untrusted"`로
  명시된 project는 Codex 0.150부터 native `AGENTS.md`를 읽지 않으므로
  `verify`가 FAIL한다. trust entry가 없으면 native 로드는 되지만 repo의
  `.codex/config.toml`이 무시되므로 WARN으로 보고한다.
- `project_doc_fallback_filenames`는 `[]` 또는 생략, `project_root_markers`는
  두지 않는다. 위반하면 `verify`가 실패한다. `AGENTS.md`는
  `project_doc_max_bytes`(기본 32768 bytes) 이하여야 한다. trusted project에서는
  worktree root부터 세션 시작 디렉터리까지의 `.codex/config.toml`이 user
  config보다 우선하므로, `verify`는 그 체인에서 두 key를 합성해 판정한다.
...
Kiro 경로는 experimental이며 Claude/Codex 안정 보증과 분리한다.
...
- launcher는 `kiro-cli settings list --format json`으로
  ...
  중단한다. `verify`는 Kiro 설정 문제를 WARN으로 보고하며, Kiro를 target으로
  쓸 때만 이 WARN을 release gate로 본다.
...
- rule source가 symlink, non-UTF-8, NUL 포함이면 hook은 해당 rule의 text를
  ...
  계속 들어간다.
- hook 주입 예산(Codex/Kiro 로컬 32,768자)을 넘으면 잘린 전문 대신 notice가
  들어간다. rule 파일을 줄이거나, 세션마다 필요하지 않은 내용은 일반 문서로
  옮겨 명시적으로 열게 한다.
